...
public final class SearchResultsCardFactory {

    private static volatile Image defaultCover;

    private SearchResultsCardFactory() {}

    public static Image defaultCover() {
        Image cached = defaultCover;
        if (cached != null && !cached.isError()) return cached;

        synchronized (SearchResultsCardFactory.class) {
            cached = defaultCover;
            if (cached == null || cached.isError()) {
                cached = MusicCardHelper.loadDefaultCover();
                defaultCover = cached;
            }
            return cached;
        }
    ...
    public static Parent createCard(String id,
                                    ...
                                    Consumer<String> onArtistClick,
                                    boolean playlistCard) throws IOException {
        Image cover = localCover != null ? localCover : defaultCover();
