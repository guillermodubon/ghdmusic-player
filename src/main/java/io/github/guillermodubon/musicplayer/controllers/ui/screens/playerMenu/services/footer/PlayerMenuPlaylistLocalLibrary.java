...
public final class PlayerMenuPlaylistLocalLibrary {
    ...
    private final AtomicLong recommendationLoadSequence = new AtomicLong();
    ...
    public PlayerMenuPlaylistLocalLibrary(PlayerMenuContext context,
                                          StartUpService svc,
    ...
    public void bindUi(VBox recContainer,
        ...
        if (recList != null) {
            ...
            PlayerMenuPlaylistLibraryRenderer.configureList(
                    recList,
                    svc,
        ...
        if (refreshButton != null) {
            PlayerMenuPlaylistLibraryRenderer.configureRefreshButton(refreshButton, () -> {
    ...
    public void clear() {
        recommendationLoadSequence.incrementAndGet();
    ...
    public void populateRecommendations() {
        ...
        long loadSequence = recommendationLoadSequence.incrementAndGet();
        List<Song> alreadyIn = playlist.getSongList() == null
                ? List.of() : new ArrayList<>(playlist.getSongList());
        List<Song> sourceSongs = svc.getSongs() == null
                ? List.of() : MusicCardHelper.snapshot(svc.getSongs());

        CompletableFuture
                .supplyAsync(() -> buildRecommendationCandidates(sourceSongs, alreadyIn), ioPool)
                .thenAccept(candidates -> Platform.runLater(() -> {
                    if (loadSequence != recommendationLoadSequence.get()
    ...
    private List<Song> buildRecommendationCandidates(List<Song> sourceSongs,
                                                       List<Song> alreadyIn) {
        ...
        List<Song> candidates = LocalSongVerifier.verifiedPlayableLocalSongs(
                        sourceSongs == null ? List.of() : sourceSongs,
                        svc,
                        manifest
                )
                .stream()
                .filter(song -> alreadyIn == null || !alreadyIn.contains(song))
    ...
    private void handleRecommendationPlay(Song song) {
        if (song == null) return;
        playbackManager.setOriginSource("Library");
